...
export function ProspectsProvider({ children }) {
  ...
  async function generateAiDemo(prospectId, prospectOverrides = {}) {
    ...
    if (!isSupabaseConfigured) {
      // Call our local DemoForge generator API on port 3000
      try {
        const genResp = await fetch('http://localhost:3000/api/generate-demo', {
          method: 'POST',
          ...
          body: JSON.stringify({
            prospect: {
              ...
              brand_logo_url: prospect.brand_logo_url,
              services: prospect.demo_copy?.split('\n').filter(l => l.includes('Services') || l.includes('services')).flatMap(l => l.split(/[,;]/).map(s => s.trim())).filter(Boolean) || [],
            },
          }),
        })

        if (!genResp.ok) {
          throw new Error(`Generator API returned ${genResp.status}`)
        }

        const genData = await genResp.json()
        const previewUrl = genData.preview_url || 'https://9846440675e95403841da54a5c4a4e4b.ctonew.app'

        const generated = {
          research_summary: 'Demo site generated locally by DemoForge AI.',
          ...
          design_notes: genData.success ? 'Custom-generated demo with unique color scheme and layout.' : 'Local preview demo copy only',
          designed_site: { html: '', css: '', summary: 'Generated by DemoForge AI', style_direction: 'Modern clean' },
          ...
          generation_error: genData.success ? '' : genData.output || '',
        }
        const result = await updateProspect(prospectId, {
          ...
          generation_provider: generated.generation_provider || generated.ai_provider || 'demoforge',
          generation_error: generated.generation_error || '',
          preview_url: previewUrl,
          deployment_status: 'live',
        })
        if (!result.error) await addActivity(prospectId, { type: 'AI Demo', note: `Generated DemoForge AI demo: ${previewUrl}` })
        return result
      } catch (fetchErr) {
        // Fallback: if API call fails, use the basic mock
        const mockGenerated = {
          research_summary: 'Local preview mode: connect Supabase + OpenAI/secrets to generate research-backed demo copy.',
          hero_headline: `${prospect.business_name} — modern local website demo`,
          hero_subheadline: `A clean demo website designed to help ${prospect.business_name} explain services and turn visitors into calls or messages.`,
          primary_cta: prospect.phone ? 'Call Now' : 'Request a Quote',
          secondary_cta: 'View Services',
        ...
        const result = await updateProspect(prospectId, {
          demo_status: 'building',
          status: prospect.status === 'research' ? 'demo_ready' : prospect.status,
          demo_brief: mockGenerated.hero_subheadline,
          demo_copy: formatAiDemoCopy(mockGenerated),
          demo_notes: mockGenerated.design_notes,
          ai_research_summary: mockGenerated.research_summary,
          ai_source_links: '',
          ai_generated_at: new Date().toISOString(),
          demo_site_html: mockGenerated.designed_site?.html || '',
          demo_site_css: mockGenerated.designed_site?.css || '',
          demo_design_summary: mockGenerated.designed_site?.summary || mockGenerated.design_notes || '',
          demo_style: mockGenerated.designed_site?.style_direction || mockGenerated.brand_profile?.design_direction || 'Modern clean',
          brand_logo_url: mockGenerated.brand_profile?.logo_url || prospect.brand_logo_url || '',
          brand_profile: mockGenerated.brand_profile || null,
          generation_provider: mockGenerated.generation_provider || mockGenerated.ai_provider || 'local_preview',
          generation_error: mockGenerated.generation_error || '',
        })
