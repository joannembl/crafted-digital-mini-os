  ...
  <main id="top">
    <section class="hero">
      <div class="hero-grid shell">
        <div class="hero-copy">
          <p class="eyebrow">${escapeHtml(category)} · ${escapeHtml(theme.key.replaceAll('-', ' '))}</p>
          <h1>${escapeHtml(headline)}</h1>
          <p class="lede">${escapeHtml(subheadline)}</p>
          <div class="actions">
            <a class="button primary" href="${ctaHref}">${escapeHtml(primaryCta)}</a>
            <a class="button secondary" href="#services">Explore the demo</a>
          </div>
        </div>
        <aside class="feature" aria-label="Website concept visual">
          <span class="feature-kicker">${escapeHtml(trustLine)}</span>
    ...
    <section id="services" class="section shell">
      <div class="section-title">
        <div><p class="eyebrow">What customers see first</p><h2>Clear reasons to choose ${safeName}.</h2></div>
    ...
    <section id="story" class="section shell story">
